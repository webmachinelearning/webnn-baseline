...
describe('test batchNormalization', function() {
  function testBatchNorm(
    ...
    if (scale) {
      options.scale = new Tensor(scale.shape, scale.data);
    }
    if (bias) {
  ...
  it('batchNormalization 3D input axis=0', function() {
    ...
    testBatchNorm(input, mean, variance, expected, scale, bias, {epsilon: 1e-3, axis: 0});
  });
  ...
  it('batchNormalization 3D input axis=2', function() {
    ...
    testBatchNorm(input, mean, variance, expected, scale, bias, {epsilon: 1e-3, axis: 2});
  });
  ...
  it('batchNormalization with epsilon', function() {
    ...
    const mean = {
      shape: [3],
      data: [0.3432895, 1.0855169, 1.8725895],
    };
    const variance = {
      shape: [3],
      data: [0.601868, 0.86580527, 0.38809904],
    };
    const scale = {
      shape: [3],
      data: [0.17215693, -0.7909758, 0.12456307],
    };
    const bias = {
      shape: [3],
      data: [0.5280557, -1.4475446, 0.1760742],
    };
    const expected = [
